...
def download_from_yadisk():
    ...
    api_url = f"https://cloud-api.yandex.net/v1/disk/public/resources/download?public_key={urllib.parse.quote(YADISK_PUBLIC_KEY)}"
...
def parse_remarks(xlsx_bytes):
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        ...
        # Строка акта — содержит номер акта (ЧШ- или ДД-)
            ...
            if row[1]:
                try:
                    if isinstance(row[1], (datetime, date)):
                        current_act_date = row[1].strftime("%Y-%m-%d") if hasattr(row[1], 'strftime') else str(row[1])
                    else:
        ...
        text = col0
        risk = col2 if col2 and col2 not in ("None", "") else ""
        
        # Столбцы сроков и комментариев
...
if __name__ == "__main__":
    try:
        ...
        if not remarks:
            print("Замечания не найдены, прерываю")
            sys.exit(1)
